...
- The `/stats` slash command (backed by `~/.claude/skills/stats/session_stats.py`)
  summarizes the Claude Code build sessions for this repo (token usage,
  prompt/turn counts, active vs. idle time, and estimated API cost) from the
  local transcripts; `--md` writes a Markdown report (see
...
`python3 ~/.claude/skills/stats/session_stats.py --md` from the repo root.
