...
| Metric | Value |
| --- | --- |
| Output tokens (produced content) | ~3.06 M |
| Grand total tokens (mostly cached context) | ~942 M |
| Prompts typed / assistant turns | ~85 / 2,306 |
| Active time (Claude working / you prompting) | ~8h 59m (5h 42m / 3h 17m) |
| Estimated metered-API cost (Opus 4.8 rates) | **≈ $583** |

The cost is dominated by cache reads; prompt caching saved ~$4,200 (a 10×
discount) versus billing that context as fresh input. At a flat Claude Code
subscription this build is effectively included, and the metered ~$583 is
roughly **4–14× cheaper** than the 3–10 engineer-days the equivalent hand-built
