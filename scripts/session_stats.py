...
def write_markdown(path, files, n_rows, outp, inp, cc, cr, total, prompts,
                   turns, events, active, claude, user, idle, idle_min) -> None:
    lines = [
        "# Build session stats",
        "",
        "_Generated by `scripts/session_stats.py` from the local Claude Code "
        "transcripts for this repo. Cache-read tokens dominate the total — that "
        "is the growing context re-read at the cheap cached rate on each turn, "
        "not fresh work; output tokens reflect produced content._",
        "",
        f"- **Transcripts:** {len(files)} file(s), {n_rows:,} records",
        "",
        "## Tokens",
        "",
        "| Category | Tokens |",
    ...
    if events:
        lines += [
            ...
            "| Metric | Value |",
            "| --- | --- |",
            f"| Session start | {events[0].astimezone():%Y-%m-%d %H:%M %Z} |",
            f"| Session end | {events[-1].astimezone():%Y-%m-%d %H:%M %Z} |",
